...
`POST /v1/jobs/{id}/answer` answers the explicit question on a blocked job. `POST /v1/jobs/{id}/steer` accepts `{"text":"..."}` only while a job is `starting` or `running`; blocked jobs must use `answer`, and all other lifecycle states return conflict. Steering input is persisted and queued in request order. The supervisor delivers it through the active backend's atomic prompt path (tmux bracketed paste or Herdr `agent.prompt`). Answers retain harness semantics: structured Pi blocks are next-message answers, while an unstructured Herdr screen dialog uses only deliberate `agent.send_keys` input. Accepting a steer emits a durable `job.progress` event noting that it is queued for delivery.
