...
test("manifest is exact v1 and points only into the Golem contribution", () => {
  ...
  for (const app of ["golemd", "golem-familiar-render"])
  ...
  expect(manifest).toContain('env = { GOLEM_HOOK_EXTENSION = "${plugin_root}/integrations/pi/agent-hooks/index.ts" }');
  expect(manifest).toContain('GOLEM_ENDPOINT = "http://127.0.0.1:7337"');
  expect(manifest).not.toContain('GOLEM_HOST');
